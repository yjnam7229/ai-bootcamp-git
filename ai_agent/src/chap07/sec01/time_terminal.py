...
while True:
    ...
    ai_response = get_ai_response(messages, tools=tools)  # 첫번째 요청은 function calling 방식
    ...
    if tool_calls:                               # tool_calls가 있는 경우
        for tool_call in tool_calls:             # 여러 도시의 시간을 한 번에 대답할 수 있게 하기
        ...
        ai_response = get_ai_response(messages, tools=None) # 두번째 요청은 fuction call을 요청하지 못하도록 셋팅, 다시 GPT 응답 받기
        ai_message = ai_response.choices[0].message
