...
def encode_image(image_path):
    with open(image_path, "rb") as image_file:
        return base64.b64encode(image_file.read()).decode("utf-8") # 이미지 데이터를 텍스트화
...
def image_quiz(image_path, n_trial=0, max_trial=3):
    ...
    content = response.choices[0].message.content  # 이미지 응답 데이터
...
eng_dict = []
no = 1 # 문제 번호를 위해 선언
for g in glob('../data/images/*.jpg'):  # ② 해당 폴더안의 jpg 확장자를 가진 파일들을 읽어 옴
    q, is_suceed = image_quiz(g)  # q : 문제
    ...
    txt += f'![image]({filename})\n\n'   # ![image] 마크다운 : 입력으로 파일명을 읽어 와서 ![image] 위치에 이미지를 넣어 줌
