...
# SQLAlchemy의 모델 기본 클래스 선언, 이 클래스를 상속받아 데이터베이스 테이블을 정의할 수 있음.
Base = declarative_base()

# Base 클래스를 상속받는 User 클래스 선언
class User(Base):
    # 'users' 테이블 정의
    __tablename_ = 'users'
...
# Pydantic 모델을 정의함, 이 모델은 클라이언트로부터 받은 데이터의 유효성을 검사하는데 사용됨
...
# 데이터베이스 엔진을 사용하여 모델을 기반으로 테이블을 생성함(테이블이 없는 초기 상태)
Base.metadata.create_all(bind=engine)

# FastAPI 애플리케이션에 모델 통합(초기화)
app = FastAPI()

@app.get('/')
def read_root():
    return {'message': 'Hello, World!'}      # 루트 경로에 접근했을 떄 메시지를 반환
...
# uvicorn main:app --reload 서버 재시작
